...
uchar code FONT3[] = {0x0F,0,0x0F,0,1,2,4,0}; 					// ¤ô
...
void main(void)
{
   ...
   while (1) {      
      LCD_Cmd(0x08);for(i=12;i>0;i--) Delay(65000);  // D=0Ãö³¬Åã¥Ü¹õ    
      LCD_Cmd(0x0C);for(i=12;i>0;i--) Delay(65000);  // D=1¶}©lÅã¥Ü¹õ  
	}
}
...
{
	uchar i;
   LCD_Cmd(0x40); //«ü¦VCGram
	for (i=0; i<40; i++)
		Send_Data(FONT1[i]);		//±N¸ê®Æ¼g¨ìCGram
}
